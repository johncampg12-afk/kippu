...
let FirmaElectronicaService = class FirmaElectronicaService {
    async firmarXml(xmlPath, certificadoPath, passwordCertificado) {
        if (!fs.existsSync(xmlPath)) {
            throw new common_1.BadRequestException(`Archivo XML no encontrado: ${xmlPath}`);
        }
        if (!fs.existsSync(certificadoPath)) {
            throw new common_1.BadRequestException(`Certificado no encontrado: ${certificadoPath}`);
        }
        try {
    ...
    validarCertificado(certificadoPath, password) {
        try {
            const p12Buffer = fs.readFileSync(certificadoPath);
            const p12Asn1 = forge.asn1.fromDer(p12Buffer.toString('binary'));
...
exports.FirmaElectronicaService = FirmaElectronicaService = __decorate([
